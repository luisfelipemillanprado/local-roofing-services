...
const base =
  "group inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 transition-all duration-300";

const variants: Record<ButtonVariant, string> = {
  primary: "bg-primary shadow-md shadow-shade/40 hover:-translate-y-0.5",
  dark: "bg-contrast hover:-translate-y-0.5",
  ghost: "border border-white/30 bg-white/5 backdrop-blur hover:bg-white/15",
};
...
export const Button = ({
  ...
  return (
    <Link href={href} className={`${base} ${variants[variant]} ${fullWidth ? "w-full" : ""}`}>
